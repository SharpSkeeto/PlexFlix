...
using MFMMediaPlayer.MFMediaEngine;
using Microsoft.UI;
...
using static MFMMediaPlayer.MFMediaEngine.MediaEngineTools;
//using D3D11;

namespace MediaPlayerEngineWrapper;
...
public class CMediaEngine : IMFMediaEngineNotify, IMFTimedTextNotify, IDisposable // , System.ComponentModel.INotifyPropertyChanged
{
	...
	MediaPlayerButton buttonPlayPause = null!;
	MediaPlayerButton buttonStop = null!;
	MediaPlayerButton buttonRepeat = null!;
	MediaPlayerButton buttonFullscreen = null!;
	MediaPlayerButton buttonSound = null!;
	MediaPlayerButton buttonRewind = null!;
	MediaPlayerButton buttonForward = null!;
	...
	public HRESULT Initialize(IntPtr hWnd, ME_MODE mode, Microsoft.UI.Xaml.FrameworkElement container, Windows.UI.Color borderColor)
	{
		...
		if (!m_bInitialized)
		{
			...
			if (m_pMediaEngineClassFactory != null)
			{
				...
				//string sImageFile = sDirectory + @"Assets\MyPlex-Logo.png";
				//string sLogoURL = $"file://{sImageFile}";

				var t = Task.Run(async () => await GetLogo());
				string sLogoURL = t.Result;
				...
				buttonPlayPause = new MediaPlayerButton(false, false)
				{
					Width = 40,
					Height = 40,
					HorizontalAlignment = HorizontalAlignment.Center,
				...
				// not really rewind, so-to-speak...   more like playing backwards with playbackrate
				// really needs to use seek... seek back 10s, maybe
				buttonRewind = new MediaPlayerButton()
				{
					Width = 40,
					Height = 40,
					HorizontalAlignment = HorizontalAlignment.Center,
				...
				buttonStop = new MediaPlayerButton(false, false)
				{
					Width = 40,
					Height = 40,
					HorizontalAlignment = HorizontalAlignment.Right,
				...
				// not really forward, so-to-speak...   more like playing fastforward with playbackrate
				// really needs to use seek... seek forward 10s, maybe
				buttonForward = new MediaPlayerButton()
				{
					Width = 40,
					Height = 40,
					HorizontalAlignment = HorizontalAlignment.Center,
				...
				buttonRepeat = new MediaPlayerButton(false, false)
				{
					Width = 40,
					Height = 40,
					HorizontalAlignment = HorizontalAlignment.Center,
				...
				buttonFullscreen = new MediaPlayerButton(false, false)
				{
					Width = 40,
					Height = 40,
					HorizontalAlignment = HorizontalAlignment.Center,
				...
				buttonSound = new MediaPlayerButton(false, false)
				{
					Width = 40,
					Height = 40,
					HorizontalAlignment = HorizontalAlignment.Center,
				...
				sliderSound.Value = nVolume * 100;

				sp1.Children.Add(buttonRewind);
				sp1.Children.Add(buttonPlayPause);
				sp1.Children.Add(buttonForward);
	...
	{
		buttonSound_CImageButtonClicked(null!,null!);
	}

	public void PlayToggle()
	{
		_ = (bool)IsPlayerPaused()! ? Play(true) : Pause(true);
	}

	public void FullScreenToggle()
	{
		_ = (bool)IsFullScreen()! ? SetFullScreen(false) : SetFullScreen(true);
	}

	public void VolumeUp(double volumnChange)
	{
		if (!m_pMediaEngine.GetMuted())
		{
			var volume = m_pMediaEngine.GetVolume();
			if (volume < 1)
			{
				volume += volumnChange;
				if (volume > 1)
				...
				m_pMediaEngine.SetVolume(volume);
				sliderSound.Value = volume * 100;
			}
		}
	}

	public void VolumeDown(double volumnChange)
	{
		if(!m_pMediaEngine.GetMuted())
		{
			var volume = m_pMediaEngine.GetVolume();
	...
	public void Seek(double seekTime)
	{
		...
		if (nReadyState >= MF_MEDIA_ENGINE_READY.MF_MEDIA_ENGINE_READY_HAVE_FUTURE_DATA)
		{
			...
			{
				HRESULT hr = m_pMediaEngine.GetSeekable(out var range);
				if (hr == HRESULT.S_OK)
				{
					var currTime = m_pMediaEngine.GetCurrentTime();
					var seekAbleTime = currTime += seekTime;
					if (range.ContainsTime(seekAbleTime))
					{
						hr = Pause(false);
						if (hr == HRESULT.S_OK)
						{
	...
	{
		_ = m_pMediaEngine.SetVolume(((Slider)sender).Value * .01);
	}
	...
	public HRESULT SetFullScreen(bool bFulllScreen)
	{
		...
		if (bFulllScreen)
		{
			...
			if(defaultPresenter is not null)
			{
	...
	public int AddSubtitles(string sFile)
	{
		...
		if(m_pTimedText is not null)
		{
			hr = m_pTimedText.AddDataSourceFromUrl(sFile, null!, null!, MF_TIMED_TEXT_TRACK_KIND.MF_TIMED_TEXT_TRACK_KIND_SUBTITLES, true, out uint nTrackId);
			if (hr == HRESULT.S_OK)
			{
				return (int)nTrackId!;
			}
	...
	//private async Task<IntPtr> SetMediaPlayerLogo(IntPtr pee)
	//{
